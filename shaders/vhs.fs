...
uniform float wiggle = 0.03;
uniform float wiggle_speed = 25.0;
...
vec3 Blur(vec2 uv, float d, int samples) {
    ...
    for (int i = 0; i < samples; ++i) {
        float t = (sin(gl_FragCoord.y * 5.0 + uv.y * 5.0)) / 10.0;
        ...
        vec3 N = texture(texture0, uv + Circle(Start, float(samples), float(i)) * Scale).rgb;
        sum += N * W;
    }
    return sum;
}
...
void main() {
    vec2 uv = gl_FragCoord.xy / vec2(textureSize(texture0, 0)); // Normalize UV coordinates
    float framecount = gl_FragCoord.x + gl_FragCoord.y; // Simulate time or framecount

    float d = 0.1 - round(mod(framecount / 3.0, 1.0)) * 0.1;
    uv = jumpy(uv, mod(framecount * wiggle_speed, 7.0));

    float s = 0.0001 * -d + 0.0001 * wiggle * (sin(framecount * wiggle_speed));
    float e = min(0.30, pow(max(0.0, cos(uv.y * 4.0 + 0.3) - 0.75) * (s + 0.5) * 1.0, 3.0)) * 25.0;
    float r = (250.0 * (2.0 * s));
    uv.x += abs(r * pow(min(0.003, (-uv.y + (0.01 * mod(framecount, 5.0)))) * 3.0, 2.0)) * wiggle;
